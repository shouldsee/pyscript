...
def gillespie_MM(s0,t_obs_out,param):
    ...
    while t < t_final:
        ...
        types=['ES_bind','ES_unbind','ES_to_P']
        ...
        rate_ES_bind = kf1*E*S
        rate_ES_unbind = kb1*ES
        rate_ES_to_P = kf2*ES
        ...
        rates=[rate_ES_bind, rate_ES_unbind, rate_ES_to_P]
        ...
        if event_type=='ES_bind':
            E+=-1
            S+=-1
            ES+=1
        elif event_type=='ES_unbind':
            E+=1
            S+=1
            ES+=-1
        elif event_type=='ES_to_P':
            P+=1
            ES+=-1
            E+=1
        else:
...
#s_obs=gillespie_logistic(s0,t_obs,param)
...
output=multiprocess(gillespie_MM,   1,     fn_args=(s0,t_obs,param) )
