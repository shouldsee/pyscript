...
output=multiprocess(gillespie_gene_expression, 100, fn_args=(s0,t_obs,param))
...
# average over columns to get average of trajectories at each time point
av_p_obs=np.average(p_obs,1)

# this is a check that we averaged the table correctly
# the output array should contain 30000 timepoints
print len(av_p_obs)
...
ax_P.hist(p_end, bins=10)
ax_P.set_title("Histogram of P copy number at t=30000, (100 runs)")
ax_P.set_xlabel('#  proteins')
fig1.show()

fig2=plt.figure()
ax_mRNA=fig2.add_subplot(1,1,1)
ax_mRNA.hist(mRNA_end, bins=np.arange(-0.5,40.6,1.0))
ax_mRNA.set_title("Histogram of mRNA copy number at t=30000, (100 runs)")
...
# add "super title" over plot set
fig4.suptitle('Stochastic Gene Expression Model')
ax_P=fig4.add_subplot(1,1,1)
ax_P.set_ylabel('#Protein copies')
ax_P.set_xlabel('time (s)')
ax_P.plot(t_obs, av_p_obs, 'g-')
ax_P.plot(t_obs, p_obs[:,0:5], ':')
